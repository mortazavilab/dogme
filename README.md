...
- **FASTQ seqspec generation:** DOGME can render, upgrade, format, and validate a seqspec artifact whenever it generates a FASTQ from an unmapped BAM. This works for bulk and single-cell configurations.
- **Explicit single-cell opt-in:** `params.singleCell` defaults to `false`; single-cell behavior is enabled only when it is explicitly set to the boolean `true`.
...
- **Seqspec generation for FASTQs:**
  FASTQs generated by DOGME can be accompanied by a rendered, upgraded, formatted, and validated seqspec file. Configure `seqspecTemplate` and `seqspecVariables` in the Nextflow parameters; seqspec and Jinja2 are provided by the Docker/Apptainer image.
...
params {
    ...
    // singleCell must be the boolean true to enable single-cell behavior.
    singleCell = false
    singleCellKit = null
...
When DOGME creates a FASTQ from an unmapped BAM, it can generate a seqspec artifact beside the FASTQ. The artifact describes the assay geometry and is independent of any assay-specific barcode correction algorithm. This applies to bulk and single-cell FASTQ generation.
...
`singleCell` defaults to `false` and only an actual boolean `true` enables single-cell behavior. `singleCellKit` is metadata used for template selection and provenance; it does not itself activate single-cell processing. A pre-rendered external spec may be supplied with `params.seqspec` for workflows that consume existing FASTQs.
